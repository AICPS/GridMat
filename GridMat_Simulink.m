...
function setup(block)
  ...
  block.InputPort(1).Dimensions  = -1;  % inherited
  ...
  [pathfile,namefile,ext] = fileparts(block.DialogPrm(3).Data);
  if(isempty(ext))
      ext='m';
  end
  if(isempty(pathfile))
  ...
  [input_size, output_size] = generateIOL2([pathfile '\' namefile '.' ext]);
  block.OutputPort(2).DatatypeID  = 0; % double
  block.OutputPort(2).Dimensions  = input_size;
...
[pathfile,namefile,ext] = fileparts(block.DialogPrm(2).Data);
if(isempty(ext))
    ext='glm';
end
if(isempty(pathfile))
...
[starttime,stoptime]=extractdate([pathfile '\' namefile '.' ext]);
socket = tcpip('localhost', 6267, 'NetworkRole', 'client');
fclose(socket);
dos(['SET PATH=%%PATH%%;',path,' & cd ',pathfile,' & gridlabd ',namefile,'.',ext,' --verbose --server --define pauseat="',datestr(addtodate(starttime,1,'second'),31),'" &']);global socket;
pause(6);
fopen(socket);
%endfunction
...
% block.OutputPort(1).Data =  block.InputPort(1).Data(1);
global socket;
global starttime;
global stoptime;
% if(block.CurrentTime~=0)
    u=block.InputPort(1).Data;
    eval([block.DialogPrm(3).Data,'_o']);
% end
time=addtodate(starttime,block.CurrentTime,'second');
block.OutputPort(1).Data=time;
date=datestr(time,31)
readValue(socket,['control/pauseat=',date]);
state='NONE';
while (strcmp(state,'PAUSED')==0)
    if(time>stoptime || strcmp(socket.status,'closed')==1)
...
eval([block.DialogPrm(3).Data,'_i']);
